/*
  Consolidated: Task Posting + Moderation (idempotent, enum-safe, Supabase-ready)

  What this does:
  - Ensures moderation enum/columns exist on public.tasks
  - Ensures common task columns exist (category TEXT, etc.)
  - Ensures updated_at trigger exists
  - (Optional safety) Creates public.task_category enum IF NOT EXISTS (to satisfy legacy objects),
    but the RPC below NEVER references it.
  - Rebuilds RPC public.moderate_task_and_save(...) with TEXT params only
  - Fixes/sets minimal RLS policies using auth.uid()
  - Grants execute; reloads PostgREST schema
  - Verifies RPC body contains NO 'uid(' and NO '::public.task_category' casts
*/
...
--------------------------------------------
-- Phase 1: Moderation enum + task columns --
--------------------------------------------
DO $$
BEGIN
...
-------------------------------------------------------------
-- Phase 2: Touch trigger for updated_at (create or replace) --
-------------------------------------------------------------
...
-----------------------------------------------------------------------
-- Phase 3 (Optional safety): Create legacy enum public.task_category --
-----------------------------------------------------------------------
